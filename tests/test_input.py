...
if TYPE_CHECKING:
    from autorequests.input import ParsedInput


@pytest.mark.parametrize("sample,expected", list(powershell_examples.items()))
def test_parse_powershell_to_method(sample: str, expected: ParsedInput) -> None:
    assert parse_powershell(sample) == expected


@pytest.mark.parametrize("sample,expected", list(fetch_examples.items()))
def test_parse_fetch_to_method(sample: str, expected: ParsedInput) -> None:
